...
| T-157 | **5 Dynamic grid safety gate tests + runtime contract verification**: (1) `test_dynamic_grid_disabled_no_buy` — grid NIE tworzy BUY gdy `dynamic_grid_enabled=False`. (2) `test_dynamic_grid_legacy_system_no_buy` — grid NIE tworzy BUY gdy `trading_system=legacy`. (3) `test_dynamic_grid_one_plan_per_symbol` — grid tworzy maksymalnie jeden `GridPlan` per symbol (deduplikacja). (4) `test_dynamic_grid_plans_cleanup_on_removal` — `active_grid_plans` znika/aktualizuje się gdy symbol wypada z universe. (5) `test_legacy_signal_path_blocked_on_dynamic_grid` — legacy `best-opportunity/signal path` NIE tworzy pending BUY przy `trading_system=dynamic_grid`. Wszystkie testy mają reason_code diagnostyki. | `tests/test_trading_dynamic_grid_safety.py` | Safety: Walidacja, że dynamic_grid isolation jest hermetyczna. Testy pokrywają blokady i deduplikację. | DONE ✅ (12/12 tests PASSED) |

| T-158.1 | **Legacy Exit Guard for Dynamic Grid Positions**: Implementuj guard w `_check_exits()` kolektora, aby pozycje zarządzane przez dynamic_grid NIE były zamykane przez legacy exit engine (stop loss / take profit / trailing stop). (1) Wyciągnij `is_dynamic_grid_position()` helper na poziom modułu w `backend/collector.py`. (2) Dodaj guardy w `_check_exits()` dla trzech warstw: hard SL, trailing stop, TP (linie 4796-4804, 4906-4914, 4997-5005). (3) Zweryfikuj testy: 19/19 PASSED (`test_trading_legacy_exit_guard.py`). (4) Sprawdź logowanie: `LEGACY_EXIT_GUARD` pojawia się w logach gdy grid position osiąga level exit. (5) Nie blokuje: emergency kill switch, manual closes. | `backend/collector.py`, `tests/test_trading_legacy_exit_guard.py` | Bezpieczeństwo: dynamic_grid pozycje NIE są niezamiernie zamykane przez stary engine. Zysk: grid strategia realizuje własne exit logic. Ryzyko: BRAK (guard jest pasywny i tylko pomija legacy path). | DONE ✅ (19/19 tests PASSED) |

| T-159 | **Quicktunnel auto-restart on network change**: Dodaj monitoring fingerprintu sieci w `scripts/run_quicktunnel.sh`, aby po zmianie route/IP process restartował cloudflared i generował nowy `trycloudflare.com` link dla frontend + overlay. (1) fingerprint sieci: default route + route to internet + global IPv4 + public IP. (2) restart obu tunnel processów po zmianie. (3) zapis nowego URL do `/tmp/rldc_tunnel_runtime.json` i `/home/rldc/.rldc_runtime/public_urls.txt`. (4) Monitor interval: domyślnie 20s, konfigurowalny przez `NETWORK_MONITOR_INTERVAL_SECONDS`. (5) Walidacja: `bash -n` PASS, service restart PASS, runtime pokazuje świeże URL-e. | `scripts/run_quicktunnel.sh` | UX/operacje: trycloudflare sam odświeża link po zmianie sieci bez ręcznego restartu. Ryzyko: niskie — mechanizm tylko restartuje lokalny tunnel, nie zmienia backendu. | DONE ✅ (restart service + nowy URL wygenerowany) |

| T-158 | **Dynamic Grid Live Stabilization & Safety Monitoring**: Kontynuuj live z `trading_mode=live`, `trading_system=dynamic_grid`, `dynamic_grid_enabled=True`. Monitoruj przez kilka pełnych cykli collectora: (1) active_grid_plans aktualizacja dla wielu par. (2) Brak duplikatów planów/zleceń per symbol. (3) Legacy BUY NIE tworzy wejść. (4) Każda decyzja gridu ma reason/log. (5) Live exposure within gates. (6) Grid PnL tracking. (7) Rollback ready: `dynamic_grid_enabled=False` albo `trading_system=legacy`. Endpoint `/api/account/dynamic-grid-status` pokazuje `active_grid_plans`, `plans_count`, `total_positions_from_grid`, `grid_pnl`. Checklist: active_grid_plans not empty, multiple symbols, legacy blocked, PendingOrder strategy_name set, risk gate works, kill_switch works. | `backend/collector.py`, `backend/routers/account.py`, `backend/trading/dynamic_grid.py`, logs | Live validation: Kod działa, safety gates aktywne, obserwujemy realny PnL i exposure. Rollback: instant kill via toggle. | OPEN (PHASE 1+6 — LIVE STABILIZATION; T-158.1 COMPLETE) |

| T-155 | **Phase 2 Step 2 — Grid entry/exit orchestration**: Implementuj `backend/trading/grid_orchestration.py` z `orchestrate_grid_entries()` (place BUY na buy_levels) i `orchestrate_grid_exits()` (manage TP/SL na sell_levels). Integruj z collector `_screen_entry_candidates()` i `_check_exits()`. Zachowaj exit-first philosophy. | `backend/trading/grid_orchestration.py` (NEW, ~160 lines), `backend/collector.py` (integration), `tests/test_grid_orchestration.py` | Realne grid trades per pair. Critical dla Phase 2 completion. | OPEN (NEXT — PHASE 2 AFTER SAFETY) |
